...
func TestPgidSet_add_remove(t *testing.T) {
	var s PgidSet
	s.Add(100)
	...
	s.Remove(200)
	if len(s) != 1 || s.Has(200) {
		t.Fatal("Set must contain exactly one element after removal")
	}

	s.Remove(100)
	if len(s) != 0 || s.Has(100) {
		t.Fatal("Set must be empty after removing all elements")
	}
}
